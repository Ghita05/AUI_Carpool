...
const RIDE = { departure:'AUI Main Gate',destination:'Fez Airport',departureTime:'14:00',driver:'Ghita Nafa',vehicle:'Dacia Logan',availableSeats:4,price:50,stops:['AUI Main Gate','Ifrane Marché','Hay Riad'] };
  ...
  return (
    <SafeAreaView style={st.safe} edges={['top']}>
      ...
      <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':'height'}>
        <ScrollView style={st.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Card style={{marginTop:Spacing.lg}}>
            <View style={st.summaryRoute}><Ionicons name="location" size={14} color={Colors.primary}/><Text style={st.summaryRouteText}>{RIDE.departure} → {RIDE.destination}</Text></View>
            <View style={st.divider}/>
            <View style={st.summaryGrid}>
              <View style={st.summaryItem}><Text style={st.summaryLabel}>Departure</Text><Text style={st.summaryVal}>{RIDE.departureTime}</Text></View>
              <View style={st.summaryItem}><Text style={st.summaryLabel}>Driver</Text><Text style={st.summaryVal}>{RIDE.driver}</Text></View>
              <View style={st.summaryItem}><Text style={st.summaryLabel}>Vehicle</Text><Text style={st.summaryVal}>{RIDE.vehicle}</Text></View>
            </View>
          </Card>
          ...
          {/* Stop Selection */}
          <Card>
            <View style={st.rowBetween}><Text style={st.cardLabel}>Your Stop</Text><Text style={st.subLabel}>{RIDE.stops.length} stops available</Text></View>
            <Text style={st.stopsHint}>Select where you want to board</Text>
            <View style={st.pillRow}>
              {RIDE.stops.map(stop=>(<TouchableOpacity key={stop} style={[st.pill,selectedStop===stop&&st.pillActive]} onPress={()=>setSelectedStop(stop)}><Text style={[st.pillText,selectedStop===stop&&st.pillTextActive]}>{stop}</Text></TouchableOpacity>))}
            </View>
          ...
          {/* Total */}
          <Card>
            <View style={st.totalRow}><Text style={st.totalLabel}>{seats} seat{seats>1?'s':''} × {RIDE.price} MAD</Text><Text style={st.totalVal}>{total} MAD</Text></View>
      ...
      <View style={st.bottomBar}>
        <TouchableOpacity style={st.confirmBtn} onPress={()=>navigation.navigate('BookingConfirmation',{seats,stop:selectedStop,luggage,total})}>
...
const st = StyleSheet.create({
  ...
  summaryRoute:{flexDirection:'row',alignItems:'center',gap:6},
  summaryRouteText:{fontSize:Typography.md,fontFamily:'PlusJakartaSans_600SemiBold',color:Colors.textPrimary},
  divider:{height:1,backgroundColor:Colors.border,marginVertical:Spacing.md},
  summaryGrid:{flexDirection:'row',flexWrap:'wrap',gap:Spacing.md},
  summaryItem:{width:'45%'},
  summaryLabel:{fontSize:10,fontFamily:'PlusJakartaSans_600SemiBold',color:Colors.textSecondary,letterSpacing:.5},
  summaryVal:{fontSize:13,fontFamily:'PlusJakartaSans_600SemiBold',color:Colors.textPrimary,marginTop:2},
  rowBetween:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:8},
  cardLabel:{fontSize:Typography.md,fontFamily:'PlusJakartaSans_700Bold',color:Colors.textPrimary},
  subLabel:{fontSize:Typography.sm,color:Colors.textSecondary},
  ...
  stepBtnDisabled:{borderColor:Colors.divider},
  stepperVal:{fontSize:22,fontFamily:'PlusJakartaSans_700Bold',color:Colors.textPrimary,minWidth:28,textAlign:'center'},
  stopsHint:{fontSize:11,color:Colors.textSecondary,marginBottom:8},
  pillRow:{flexDirection:'row',flexWrap:'wrap',gap:8},
  pill:{paddingHorizontal:14,paddingVertical:8,borderRadius:Radius.full,borderWidth:1,borderColor:Colors.border,backgroundColor:Colors.background},
  ...
  totalLabel:{fontSize:14,fontFamily:'PlusJakartaSans_500Medium',color:Colors.textSecondary},
  totalVal:{fontSize:16,fontFamily:'PlusJakartaSans_700Bold',color:Colors.primary},
  bottomBar:{padding:Spacing.lg,paddingBottom:Spacing.xl,backgroundColor:Colors.surface,borderTopWidth:1,borderTopColor:Colors.border,alignItems:'center'},
  confirmBtn:{width:'100%',height:52,backgroundColor:Colors.primary,borderRadius:Radius.md,alignItems:'center',justifyContent:'center'},
  confirmBtnText:{fontSize:Typography.md,fontFamily:'PlusJakartaSans_700Bold',color:'#fff'},
  disclaimer:{fontSize:11,color:Colors.textSecondary,marginTop:8},
});
